...
# CobaltStrike
- CobaltStrike(簡稱 CS)是一款團隊作戰滲透測試工具，分為客戶端及服務端，一個服務端可以對應多個客戶端，一個客戶端可以連接多個服務端。CS集成了端口轉發、掃描多模式端口Linstener、Windows exe 程序生成、Windows dll動態鏈結庫生成、java程序生成、 office宏代碼生成，包括站點克隆獲取瀏覽器相關信息等。

## 目錄結構
- agscript: 擴展應用腳本
- c2lint: 用於檢查profile的錯誤異常
- teamserver: 服務端程序
- cobaltstrike, cobaltstrike.jar: 客戶端程序(Java跨平台)
- logs: 目錄紀錄與目標主機的相關信息
- update, upgrade.jar: 用於更新CS
- third-party: 第三方工具
...
# 內網滲透
- 內網滲透: 再拿到webshell的時候，想辦法獲得系統信息拿到系統權限，進入到網路系統內部之後收集內部網路的各種信息，獲取內部網路有價值人員，資產信息。
- 內網滲透第一步: 內網信息收集。
## 內網環境分析
- 內網基礎環境判斷: IP、網關、 DNS、 是否能連通外網、 網路連接及端口、 本機host文件、 機器的代理、 是否在域內、 域名是甚麼
- 分析機器所處位置區域: DMZ區、 辦公區、 生產區、 核心DB等等
- 分析機器的角色: 普通web服務器、 開發服務器、 文件服務器、 代理服務器、 DNS服務器、數據存儲服務器等等
- 分析進出口流量是否連通: 
    - 協議的判斷: 常見的 TCP、 DNS、 HTTP、 ICMP等協議
    - 端口判斷: 外網VPS做監聽，內網機器測試常見端口，常見能出去的端口有 80、8080、443、 53、 110、 123等

## 工作組信息收集
- 工作組(Work Group): 是最常見最簡單最普通的資源管理模式，就是將不同電腦按功能分別列入不同組中，以方便管理
    1. 默認情況下所有電腦都處在名為WORKGROUP的工作組中
    2. 工作組資源管理模式適合於網路中電腦不多，對管理要求不嚴格的情況
    3. 他的步驟建立簡單，使用起來也好上手。大部分中小公司都採取工作組的方式對資源進行權限分配和目錄共享
    4. 相同組中的不同用戶通過對方主機的用戶名和密碼可以查看對方共享的文件夾，默認共享的是Users目錄
    5. 不同組的不同用戶通過對方主機的用戶和密碼可以查看對方共享的文件
    6. 所以工作組並不存在真正的集中管理作用，工作組裡的所有電腦都是對等的，也就是沒有伺服器和客戶機之分

## 本機信息收集
- 操作系統、權限、內網IP地址段、殺軟、端口、服務、補釘狀況、網絡環境狀況、共享、會話等
- 如果是域內主機，那麼操作系統、補釘、服務、殺軟一般都是批量安裝的

## 內網網段信息收集
- 只有找不同網段才能進行縱向滲透，否則只能橫向滲透
    1. 內網網段掃描
